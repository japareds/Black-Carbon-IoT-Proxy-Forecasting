...
class dataSet():
    def __init__(self,device='Ref_st',source='raw'):
    ...
    def load_dataFrame(self,time_freq='60min'):
        if self.device == 'Ref_st':
            path = os.path.dirname(os.path.realpath(__file__)) + '/Files/dataFrames/Reference_station/'
...
class dataSet_split():
    ...
    def create_sequences(self,input_set = 'Train', target_column='BC', sequence_length=2):
        if target_column != 'BC':
    ...
    def create_sequences_dates(self,input_set = 'Train', target_column='BC', sequence_length=2, remove_empty_sequences=False):
        """
        Alternative method for creating time sequences. Instead of using the last X entries
        it uses the last x hours that really have mneasurements in the data set
        """
        if target_column != 'BC':
             warnings.warn('The target pollutant is not Black carbon (BC)!')
        if not isinstance(sequence_length,int):
