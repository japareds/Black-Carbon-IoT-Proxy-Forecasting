...
parser.add_argument('-me','--epochs',type=int,help='Maximum number of epochs for training. The process stops after reaching this number',default=5000)
...
class BC_Forecasting_Proxy(nn.Module):
    ...
    def forward(self,x):
        ...
        if args.architecture == 'TDNN':
            ...
            output = self.TemporalLayer(x)
            output = self.activation_function(output)
            return self.regressor(output)
            
        elif args.architecture in ['RNN','GRU']:
            output,h_n = self.TemporalLayer(x)
            return self.regressor(output[:,-1,:])
        else:# LSTM
            output,(h_n,c_n) = self.TemporalLayer(x)
            return self.regressor(output[:,-1,:])
...
class BC_Predictor(pl.LightningModule):
    ...
    def training_step(self,batch,batch_idx):
        ...
        self.log_dict({'train_loss':loss,'step':self.current_epoch},prog_bar=True,logger=True,on_step=False,on_epoch=True,enable_graph=True)
    ...
    def validation_step(self,batch,batch_idx):
        ...
        y = batch['label']
        outputs = self(x)
        loss = F.mse_loss(outputs.squeeze().float(),y.squeeze().float())
        self.log_dict({'val_loss':loss,'step':self.current_epoch},prog_bar=True,logger=True,on_step=True,on_epoch=False,enable_graph=True, sync_dist=True)
        #self.logger.log_graph(self)
    ...
    def configure_optimizers(self):
        return optim.Adam(self.parameters(), lr = args.learningrate, weight_decay = args.weightDecay)
    
    def validation_epoch_end(self,outputs):
        avg_loss = torch.stack([x['val_loss'] for x in outputs]).mean()
        self.log_dict({'avg_val_loss':avg_loss,'step':self.current_epoch},prog_bar=True,logger=True,on_step=False,on_epoch=True,enable_graph=True, sync_dist=True)
...
def main(model,data_module,args):
    ...
    if args.train:
        ...
        fname = f'{args.architecture}_{args.npredictors}predictors_{args.hiddenlayers}HL_\
{args.nodeshl}nodesHL_{args.dropoutprob}pDO_{args.learningrate}lr_{args.weightDecay}wd\
_{args.epochs}Maxepochs_{args.batchsize}BatchSize_{args.seqlength}SeqL_{args.forecastHours}hForecast'
        ...
        checkpoint_callback = ModelCheckpoint(
            dirpath='checkpoints',
            filename=fname+'_{epoch:02d}_{avg_val_loss:.3f}_{train_loss:.3f}_bestCheckpoint',
            save_top_k=1,
            verbose=True,
        ...
        prog_bar_callback = RichProgressBar(leave=True,theme=RichProgressBarTheme(
